...
def generate_video_descriptions(model_name):
    ...
    # 1. Ollama Summary
    ollama_summary = call_ollama(shared_prompt, model=model_name)
    try:
        with open("video_description.txt", "w", encoding="utf-8") as f:
            f.write(f"--- Ollama ({model_name}) ---\n{ollama_summary}")
        logging.info("Saved video_description.txt (Ollama)")
    except Exception as e:
        logging.error(f"Error saving Ollama: {e}")

    # 2. OpenAI Summary
    openai_summary = call_openai(shared_prompt, model="gpt-4o")
    try:
        with open("video_description_openai.txt", "w", encoding="utf-8") as f:
            f.write(f"--- OpenAI (gpt-4o) ---\n{openai_summary}")
        logging.info("Saved video_description_openai.txt (OpenAI)")
    except Exception as e:
        logging.error(f"Error saving OpenAI: {e}")
...
def process_video(video_path, model_name, sample_rate=1):
    ...
    # Only convert if fixed file doesn't exist
    ...
    # --- Process Video Frames with Scene Detection ---
    import cv2
    cap = cv2.VideoCapture(video_path)
    
    frame_idx = 0
    prev_hist = None  # To store the previous frame's histogram
    last_logged_time = -10  # To ensure we force a log every few seconds
    
    # Threshold: 0.90 means if frames are 90% similar, skip them.
    SIMILARITY_THRESHOLD = 0.90 
    FORCE_LOG_INTERVAL = 5.0 # Seconds

    logging.info("Starting visual analysis (Skipping duplicate frames)...")
    ...
    while True:
        ...
        frame_idx += 1
        # Skip based on sample rate first
        ...
        # 1. Convert to HSV for better color comparison
        ...
        if prev_hist is not None:
            # 3. Compare with previous frame (Correlation method)
            similarity = cv2.compareHist(prev_hist, hist, cv2.HISTCMP_CORREL)
            
            # 4. Check if we should skip
            if similarity > SIMILARITY_THRESHOLD:
                # If it's a duplicate, ONLY skip if we haven't forced a log recently
        ...
        # --- AI ANALYSIS (Only runs on unique frames) ---
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  1. report.txt (Detailed logs)")
        print("  2. video_description.txt (Ollama Summary)")
        print("  3. video_description_openai.txt (OpenAI Summary)")
