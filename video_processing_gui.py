...
def generate_video_descriptions(model_name):
    # Note: model_name argument is kept for compatibility but not used for Ollama anymore.
    # It is passed to OpenAI if specified, otherwise defaults to gpt-4o.
    ...
    openai_summary = call_openai(shared_prompt, model=model_name)
    
    output_file = "video_description.txt"
    try:
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(f"--- Generated by OpenAI ({model_name}) ---\n\n")
            f.write(openai_summary)
        logging.info(f"Description saved to {output_file}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  2. video_description.txt (OpenAI Summary)")
